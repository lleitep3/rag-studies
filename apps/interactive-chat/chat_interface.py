...
class ChatInterface:
        ...
        if result['error']:
            ...
            if result['sources']:
                ...
                for source in result['sources']:
                    print(f"   • {source['file']} ({source['lines']} linhas, score: {source['score']})")
    ...
    def _show_banner(self):
        ...
        print("\n" + "🤖" + "=" * 58 + "🤖")
        ColoredOutput.print_colored("   RAG CODE ASSISTANT - Chat Interativo", ColoredOutput.CYAN, bold=True)
        print("🤖" + "=" * 58 + "🤖")
    ...
    def _show_examples(self, show_all: bool = True):
        ...
        examples_to_show = self.example_questions if show_all else self.example_questions[:5]
        
        for i, example in enumerate(examples_to_show, 1):
            print(f"   {i}. {example}")
        
        if not show_all and len(self.example_questions) > 5:
            print(f"   ... e mais {len(self.example_questions) - 5} exemplos (digite 'examples' para ver todos)")
    ...
    def _search_keywords(self):
        ...
        ColoredOutput.print_colored("Digite as palavras-chave separadas por espaço: ", ColoredOutput.CYAN)
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"      • Palavras encontradas: {', '.join(result['matched_keywords'])}")
